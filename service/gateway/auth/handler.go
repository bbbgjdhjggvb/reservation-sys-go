// 微信 OAuth 验证流程说明:
//  1. 防止有心人随意编造 openid 进行高频访问，搞垮服务器
//  2. 限制用户必须通过微信服务号访问预约界面
//
// 验证运行逻辑:
//  1. 用户在微信服务号点击预约按钮，微信服务器发给后台服务器一个 code
//  2. 后端的 authHdl.WeChatCallBack 用这个 code 换取用户的 openid
//  3. 然后利用 openid 生成一个 Token，重定向到预约前端网页
//  4. 网页 URL 后面带上 token，用户填写信息提交时，将信息和 token 一起发送到后台
//  5. 后台的预约处理模块根据 token 校验用户身份
